...
<nav class="bg-white border-gray-200 px-2 sm:px-4 py-2.5 rounded dark:bg-gray-900 fixed z-20 w-full" :class="{
      ...
      <div class="w-full md:block md:w-auto" id="navbar-default"
        ...
        <ul class="flex flex-col p-4 mt-4 bg-gray-50 rounded-lg border border-gray-100 md:flex-row md:space-x-8 md:mt-0 md:font-medium md:border-0 md:bg-white dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700">
          <li v-for="(navigation, index) in navigations" :key="index">
            <a :href="navigation.href" class="block py-2 pr-4 pl-3 text-light-tail-500 rounded dark:text-dark-navy-100 hover:text-light-tail-500 dark:hover:text-dark-secondary" aria-current="page">{{ navigation.name }}</a>
          </li>
